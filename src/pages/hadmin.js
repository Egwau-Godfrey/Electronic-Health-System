  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        // Initialize the passwordVisible state with default values
        const passwordVisibleDefaults = employeesData.reduce((acc, employee) => {
          acc[employee.id] = false;
          return acc;
        }, {});
        setPasswordVisible(passwordVisibleDefaults);
      } catch (error) {
    ...
    const handleSave = async () => {
        try {
            ...
            await addDoc(workersRef, {
                [formData.position]: formData.fname.toLowerCase(), // Dynamically set field based on position
